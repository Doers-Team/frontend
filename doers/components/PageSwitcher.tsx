...
const PageSwitcher = () => {
  ...
  return (
    <div className='flex items-center justify-center mt-6 p-3'>
      ...
      <Link href={urls.login}><InformationCircleIcon className={`text-fg ml-3 p-3 rounded-primary hover:text-gray-600 hover:scale-110 duration-300 ${pathname === urls.login ? 'bg-slate-200 w-18 h-18 shadow-primary' : 'w-16 h-16'}`} /></Link>
    </div>
  )
}
